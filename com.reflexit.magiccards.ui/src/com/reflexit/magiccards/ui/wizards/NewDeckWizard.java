 ...
 * with the extension "element". If a sample multi-page editor (also available as a template) is registered for the same extension, it will
 * be able to open it.
 */
public class NewDeckWizard extends NewCardElementWizard implements INewWizard {
	...
	protected CardElement doCreateCardElement(CollectionsContainer parent, String name, boolean virtual) {
		CardCollection d = new CardCollection(name + ".xml", parent, true);
		d.open();
		d.setVirtual(virtual);
		return d;
	}
}